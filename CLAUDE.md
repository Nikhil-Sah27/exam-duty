...
**Delivery channels hang off the notification emitter, not off call sites.** Any module that calls `notification.emitter.js` gets the email, WhatsApp and push copies automatically, for the types in `email.dispatcher.js`'s `EMAILABLE_TYPES`, `whatsapp.dispatcher.js`'s `WHATSAPPABLE_TYPES` and `push.dispatcher.js`'s `PUSHABLE_TYPES` — do not add a second "and also send a…" step to a service. The three channels are dispatched independently on purpose, one `onCommit` registration each: a dead SMTP host must not stop the WhatsApp message, a dropped WhatsApp session must not stop the email, and a push failure must stop neither. Two rules every channel must keep: it never throws into the caller (a dead SMTP host produces a `failed` EmailLog row, not a 500; likewise WhatsAppLog and PushLog), and it never sends from inside an open transaction. The second is enforced by `shared/utils/postCommit.js`: the emitter registers each send via `onCommit(session, …)`, and `withOptionalTransaction` flushes only after a successful commit, discarding on abort. If you add a new transactional flow, route external side effects through `postCommit` the same way. Push is the one channel that fans out — it addresses *devices*, so one notification for a teacher with a phone and a tablet is two sends, two `PushLog` rows and two outcomes.
...
**Reminder idempotency is a claimed `dedupeKey` per channel, not a "have I sent this?" check.** `reminder.service.js` derives one key — `reminder:<lead>:<teacherId>:<bucket>` — and claims it separately on each channel (`notify:` prefix on `Notification`, bare on `EmailLog`, `wa:` prefix on `WhatsAppLog`, `push:` prefix on `PushLog`), each with a unique partial index, *before* writing or sending. This is what makes the job safe to re-run, which the 15-minute cron and the manual `POST /reminders/run` both rely on. Don't add a send to the reminder path that bypasses the claim. Push claims per *device* — `pushService.sendToUser` suffixes the key with the token, `push:<key>#<token>` — for the same reason the other three are separate: devices fail independently, and one user-level claim would let the first device gate the rest.
