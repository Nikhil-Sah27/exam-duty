 ...
 */
function dutyToItem(d: Duty, roleLabel: DashboardRoleLabel): DashboardDutyItem {
...
export function normalizeRsGroupsUpcoming(
  ...
  return groupRSDutiesIntoUpcomingGroups(
    duties.filter((d) => d.status !== "cancelled")
  )
...
function dcsGroupToItem(g: DcsGroup): DashboardDutyItem {
...
export function normalizeDcsUpcoming(
  groups: readonly DcsGroup[]
): DashboardDutyItem[] {
  return groups
    .filter(
      (g) =>
        g.status === "claimed" && isUpcoming(g.schedule.date, g.schedule.endTime)
    )
    .sort(byScheduleAsc)
    .map(dcsGroupToItem);
}

export function normalizeDcsCompleted(
  groups: readonly DcsGroup[]
): DashboardDutyItem[] {
  return groups
    .filter(
